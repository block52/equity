...
type EquityResult struct {
	...
	Equity    float64 `json:"equity"`     // Win probability (0.0 - 1.0)
	TieEquity float64 `json:"tie_equity"` // Equity from ties
	Total     float64 `json:"total"`      // Total equity (wins + tie share)
}

// CalculationResult represents the complete result of an equity calculation
type CalculationResult struct {
	Results     []EquityResult `json:"results"`
...
func (c *Calculator) runSimulations(holeCards [][]types.Card, board []types.Card, deck []types.Card, cardsNeeded int) []EquityResult {
	...
	for h := 0; h < numHands; h++ {
		...
		tieEquity := float64(ties) / float64(c.simulations) / 2.0
